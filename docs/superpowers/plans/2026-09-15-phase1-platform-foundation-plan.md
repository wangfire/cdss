...
- [ ] 先写失败测试：出院时间早于入院时间时 `Visit.Create` 抛出 `DomainException`。
- [ ] 运行领域测试，确认因类型不存在而失败。
- [ ] 实现最小领域类型：`Visit.Create` 做时间校验；`CodingTask` 实现 `Start`、`Succeed`、`Retry`、`Fail`，非法状态转换抛出 `DomainException`。
- [ ] 补充 `PENDING -> RUNNING -> SUCCESS`、`RUNNING -> RETRYING`、`RETRYING -> FAILED` 和 `SUCCESS -> RUNNING` 拒绝测试。
- [ ] 运行领域测试，预期全部 PASS。
...
- [ ] 先写序列化失败测试，验证错误响应使用 `code`、`message`、`traceId`、`details` 四个 camelCase 字段。
- [ ] 运行 Application 测试，确认因 DTO 不存在而失败。
- [ ] 实现 DTO 和接口，禁止 DTO 直接暴露 EF 实体。
- [ ] 运行 Application 测试，预期 PASS。
...
- [ ] 先写失败测试，检查 `hospital`、`visit`、`coding_task`、`outbox_message`、`inbox_message`、`pipeline_trace`、`pipeline_trace_step`、`audit_log` 映射和唯一索引。
- [ ] 运行 Infrastructure 测试，确认因 DbContext 不存在而失败。
- [ ] 实现 `uniqueidentifier` 主键、`rowversion` 并发列、显式外键、`hospital_id` 隔离以及 Inbox `(message_id, consumer_name)` 唯一索引。
- [ ] 生成可重复执行的 SQL 脚本并写入一个无敏感数据的开发医院种子。
- [ ] 运行 Schema 测试，预期 PASS。

## Task 5：Outbox、Inbox 与幂等

**文件：** 创建 `OutboxMessage`、`OutboxPublisher`、`InboxMessage`、`InboxStore`、`IEventBus` 及 `OutboxInboxTests.cs`。

- [ ] 先写失败测试：业务写入与 Outbox 同事务、重复 Inbox 消息不执行两次、任务幂等键为 `task_id + pipeline_version`。
- [ ] 运行 Infrastructure 测试，确认因存储类不存在而失败。
- [ ] 使用同一个 EF 事务写业务数据和 Outbox；Inbox 重复插入返回 `false`，不执行 Handler。
- [ ] 运行测试，预期 PASS。
...
- [ ] 先写失败测试：创建任务、重复 `Idempotency-Key`、跨医院查询返回 NotFound。
- [ ] 运行 Application 测试，确认因服务和中间件不存在而失败。
- [ ] 实现创建任务：状态为 `PENDING`，写入 Outbox，并返回任务 ID 与 Trace ID；所有查询使用 `IRequestContext.HospitalId`。
- [ ] 实现 `GET /api/v1/health`、医院、患者、就诊、文书、编码任务创建、任务查询和 Trace 查询接口。
- [ ] 运行 Application 和 API 测试，预期 PASS。
...
- [ ] 先写失败测试：成功处理、瞬时失败重试、三次失败后 FAILED、Trace 步骤和审计落库。
- [ ] 运行处理器测试，确认因 Consumer/Processor 不存在而失败。
- [ ] 实现 `PENDING -> RUNNING -> SUCCESS`；重试间隔为 5/30/180 秒，耗尽后写错误码并进入 FAILED；处理前检查 Inbox。
- [ ] 配置 MassTransit 持久队列 `coding.task.created`、关联 Trace ID、重试策略和死信行为。
- [ ] 运行处理器测试，预期 PASS。
...
- [ ] 先写失败测试：传播 `X-Request-Id`、生成 `X-Trace-Id`、医院隔离、日志不包含患者姓名。
- [ ] 运行测试，确认中间件和观测扩展不存在时失败。
- [ ] 用 Serilog 结构化属性记录日志，禁止记录患者姓名、病历原文和 Prompt。
- [ ] 增加存活检查和就绪检查；就绪检查 SQL Server、Redis、RabbitMQ，存活检查只确认进程正常。
- [ ] 运行 `dotnet test`，预期全部 PASS。
...
- [ ] 先写冒烟脚本：启动 Compose，轮询 `/api/v1/health/ready`，创建开发医院和编码任务，轮询至 SUCCESS；确认 Compose 尚不存在时失败。
- [ ] 定义 SQL Server、Redis、RabbitMQ、API、Worker，使用命名卷、健康检查、开发凭据和内部网络。
- [ ] 文档化 `docker compose -f deploy/docker-compose.dev.yml up --build`、就绪地址、开发凭据和停止命令。
- [ ] 运行冒烟测试，预期 API/Worker Ready 且任务达到 SUCCESS。

## Task 10：最终验证与交付

**文件：** 更新 `README.md`，创建 `docs/superpowers/reports/2026-09-15-phase1-platform-foundation-verification.md`。

- [ ] 运行 `dotnet format HospitalAi.sln --verify-no-changes`。
- [ ] 运行 `dotnet test --configuration Release`。
- [ ] 运行 `pwsh -File tests/smoke/run.ps1`。
- [ ] 记录命令、通过/失败结果、服务版本和环境限制；明确尚未具备真实医院/AI 接入条件。
- [ ] 完成后提交：`git add .`、`git commit -m "feat: establish phase1 platform foundation"`。
